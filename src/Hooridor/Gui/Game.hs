module Hooridor.Gui.Game where
...
-- |Make game from the given game state.
makeGameFromGameState :: GameState -> (Game GuiState)
makeGameFromGameState gameState = Game initial render handleEvents update
  where
    initial = makeInitialGuiState gameState

makeInitialGuiState :: GameState -> GuiState
makeInitialGuiState state = GuiState state blank
